...
- ★ をすべて見つけるとステージクリア
- ステージが進むと硬いブロックが増える
  - ふつう(24) → **レンガ**(60, ステージ2〜) → **いわ**(130, ステージ4〜) → **ダイヤ**(300, ステージ7〜)
- **爆弾ブロック**（爆弾の絵柄）は壊すと周囲ごと吹き飛ぶ（掘るのが速くなる）
  - **爆風に巻き込まれると1.5秒動けない**ので、自分で割るときは離れること
- **色つきブロック**はその色のプレイヤーだけが壊せる。相手の弾は弾かれる
  - 機体のマークが描いてあるので、誰が壊せるか一目で分かる
  - ★ が挟まれて詰まないよう、畑の生成時に必ず到達経路を確保している
- **◆ かけら**がときどき自陣に落ちてくる。機体で受け止めるとチャージ大幅アップ
- **おじゃまむし**が畑から出てくる。ぶつかるとしばらく動けない
  - 撃ち落とすと**その場に強化のかけら**を落とす
- 強化が最大まで進むと、かけらの代わりに**巨大レーザーの球**が落ちる
  - 受け止めた瞬間、前方へ極太のレーザーを発射（最大ビームと同じ威力で、色つきブロック以外を貫通）
- 通常攻撃は**まっすぐ1発＋斜め2発の扇状**。横に広く掘れる
- 掘るとチャージが溜まり、満タンで**まっすぐ貫く1本のビーム**（3ブロックまで貫通）
